...
void Torneo::arbol_emp(BinTree<int> &a, const vector<string> &v, int l, bool &cond) {
  ...
  int val = 0;
  if (a.value()%2 == 0) val = z-a.value()+2;
  else val = z-a.value();
...
void Torneo::mod_puntos(const vector<string> &part, const Matriz &est, vector<int> &punt, const Categorias &lcat) {
  int psize = part.size();
  for (int i = 0; i < psize; ++i) {
...
void Torneo::fini_torneo(Lista_Jugadores &ljug, const Categorias &lcat) {
  ...
  mod_puntos(participantes, est_local, puntos_jugador, lcat);
...
bool Torneo::cons_esta_iniciado() const {
...
void Torneo::imprimir_puntos() const {
  int psize = participantes.size();
  for (int i = 0; i < psize; ++i) {
    cout << i+1 << '.' << participantes[i] << ' ' << ant_puntos_jugador[i] << endl;
  }
}